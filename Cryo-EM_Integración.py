...
Requiere:
    ...
    - O un archivo .mrc con mapa experimental.
"""
...
class CryoEMFitter:
    ...
    Usa refinamiento por gradiente descendente directamente,
    preservando la buena correlación inicial.
    """
    ...
    def optimize_lipid_positions(self, max_iterations=500, temperature=5):
        """
        Optimización Monte Carlo con temperatura baja para no destruir
        la buena correlación inicial. Solo se usa si se llama explícitamente.

        Args:
            max_iterations: número de pasos Monte Carlo (reducido)
            temperature: temperatura inicial MUY BAJA (5 en vez de 300)
        """
        best_correlation = self.calculate_correlation()
        print(f"  Correlación inicial Monte Carlo: {best_correlation:.4f}")
        best_structure = self._copy_structure()

        T = temperature
        for iteration in range(max_iterations):
            self._perturb_structure(amplitude=0.5)  # perturbación pequeña

            self.simulate_density()
            new_correlation = self.calculate_correlation()

            # CORRECCIÓN: aceptar solo si mejora (o con probabilidad baja si empeora)
            delta = new_correlation - best_correlation
            ...
            T *= 0.99  # enfriamiento más lento

            if iteration % 100 == 0:
                self.history['correlations'].append(best_correlation)
                print(f"  Iter {iteration:4d}: Corr={best_correlation:.4f}, T={T:.1f}")

        return best_correlation

    def _perturb_structure(self, amplitude=0.5):
        """Perturba posiciones con amplitud pequeña para no alejarse demasiado."""
        for lip in self.structure.lado_superior + self.structure.lado_inferior:
    ...
    def refine_with_gradient(self, learning_rate=0.05, steps=200):
        """
        Refinamiento fino con gradiente descendente.
        Este es ahora el método PRINCIPAL de optimización.
        learning_rate reducido y steps aumentados para mayor precisión.
        """
        print(f"\n  Iniciando refinamiento por gradiente ({steps} pasos)...")
        for step in range(steps):
            ...
            if step % 20 == 0:
                ...
                self.history['correlations'].append(correlation)
                print(f"  Paso {step:4d}: Corr={correlation:.4f}")
...
def integrate_with_membrana_fisica(membrana, cryo_em_map, optimize=True):
    ...
    print(f"Correlación inicial: {initial_corr:.4f}")

    if optimize:
        # CAMBIO DE STEPS PARA ACELERACIÓN
        fitter.refine_with_gradient(steps=50, learning_rate=0.05)
        final_corr = fitter.calculate_correlation()
        print(f"Correlación final: {final_corr:.4f}")
