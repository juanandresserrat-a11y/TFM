...
def interdigitation_map(
    ...
    """
    Penetración normalizada de colas acil trans-leaflet.

    Métrica continua: score = max(0, pen) / longitud_cadena_local
    donde pen = distancia del carbono terminal más allá del plano medio
    LOCAL (corregido por curvatura Helfrich).

    Lo (gel, SM C24): score ~0.45–0.60 | Ld: score ~0.25–0.40.
    Referencia: Chaisson, Heberle, Doktorova JCIM 2025 [9].
    """
    ...
    for lip in membrane.outer_leaflet + membrane.inner_leaflet:
        ...
        for tail in [lip.tail1, lip.tail2]:
            ...
            score_sum[ix, iy] += max(0.0, pen / chain_half)
            count[ix, iy] += 1
